...
class AnalysisConfig:
    ...
    def __post_init__(self):
        ...
        if self.recognition_config is None and self.mode in [AnalysisMode.FULL_ANALYSIS, AnalysisMode.COMPREHENSIVE]:
...
class FaceResult:
    ...
    def identity(self) -> Optional[str]:
        ...
        if self.has_identity and self.best_match:
            return getattr(self.best_match, 'person_id', getattr(self.best_match, 'identity_id', None))
        return None
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result = {
            'bbox': self.bbox.to_dict() if hasattr(self.bbox, 'to_dict') else {
...
class FaceAnalysisResult:
    ...
    def __post_init__(self):
        ...
        self.performance_breakdown = {
            ...
            "average_face_processing_time": float(self.total_time / max(self.total_faces, 1)),
            "faces_per_second": float(max(self.total_faces, 1) / max(self.total_time, 0.001))
        }
    ...
    def average_recognition_confidence(self) -> float:
        ...
        if not identified_faces:
            return 0.0
        return sum(face.recognition_confidence for face in identified_faces) / len(identified_faces)
    ...
    def get_faces_by_confidence(self, min_confidence: float = 0.8) -> List[FaceResult]:
        ...
        return [face for face in self.faces if face.recognition_confidence >= min_confidence]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            'image_shape': {
                ...
                'channels': int(self.image_shape[2]) if len(self.image_shape) > 2 else 1
            },
            ...
            'statistics': {
                ...
                'average_recognition_confidence': float(self.average_recognition_confidence)
            },
...
class BatchAnalysisResult:
    ...
    def get_summary_statistics(self) -> Dict[str, Any]:
        ...
        return {
            "detection_stats": {
                "average_time": float(np.mean(detection_times)) if detection_times else 0.0,
            ...
            "recognition_stats": {
                "average_time": float(np.mean(recognition_times)) if recognition_times else 0.0,
                "min_time": float(np.min(recognition_times)) if recognition_times else 0.0,
                "max_time": float(np.max(recognition_times)) if recognition_times else 0.0
            },
            "quality_stats": {
