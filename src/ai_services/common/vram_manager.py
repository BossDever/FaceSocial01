...
class VRAMManager:
        ...
        async with self.lock:
            try:
                ...
                if self.total_vram == 0:
                    logger.warning(
                        f"⚠️ No GPU available for model {model_id}, using CPU"
                    )
                ...
                if vram_required is None:
                    ...
                    vram_required = estimates.get(
                        model_id, 512 * 1024 * 1024
                    )  # 512MB default
                ...
                # Allocate VRAM
                if (
                    available_vram >= vram_required
                    or priority_enum == AllocationPriority.CRITICAL
                ):
                    # For CRITICAL priority, free up space if needed
                    if (
                        available_vram < vram_required
                        and priority_enum == AllocationPriority.CRITICAL
                    ):
                        freed = self._free_vram_for_critical_model(
                            vram_required - available_vram
                        )
    ...
    def _free_vram_for_critical_model(self, vram_needed: int) -> int:
        ...
        candidates = sorted(
            [
                a
                for a in self.model_allocations.values()
                if a.location == AllocationLocation.GPU
            ],
    ...
    def _log_allocation_event(
        self, event_type: str, allocation: ModelAllocation
    ) -> None:
        ...
        if len(self.allocation_history) > self.max_history_size:
            self.allocation_history = self.allocation_history[
        ...
        async with self.lock:
            try:
                ...
                status = {
                    ...
                    "available_vram": self.total_vram - self.allocated_vram,
                    "usage_percentage": (self.allocated_vram / self.total_vram * 100)
                    if self.total_vram > 0
                    else 0,
                    ...
                    "cpu_models": len(
                        [
                            a
                            for a in self.model_allocations.values()
                            if a.location == AllocationLocation.CPU
                        ]
                    ),
                    "model_allocations": {
        ...
        try:
            async with self.lock:
                ...
                for allocation in models_by_priority:
                    if (
                        allocation.priority == AllocationPriority.CRITICAL
                        and allocation.location == AllocationLocation.CPU
                    ):
                        # Try to move to GPU
