...
class FaceRecognitionService:
    ...
    def __init__(self, vram_manager=None, config: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            if (self.config.enable_gpu_optimization and 
                ...
                try:
                    ...
                    device_used = "GPU"
                    self.logger.info(f"🔥 Configured {model_type.value} for GPU with {gpu_mem_limit/1024/1024:.0f}MB limit")
        ...
        try:
            ...
            for i in range(5):
                try:
                    ...
                    self.logger.warning(f"⚠️ Warmup iteration {i+1} failed: {warmup_error}")
    ...
    async def extract_embedding(self, face_image: np.ndarray, 
                              model_name: Optional[str] = None) -> Optional[FaceEmbedding]:
        ...
        try:
            ...
            if model_name and model_name != self.current_model_type.value:
                try:
                    ...
                    self.logger.warning(f"⚠️ Unknown model: {model_name}, using current model")
    ...
    def compare_faces(self, embedding1: np.ndarray, embedding2: np.ndarray, 
        ...
        try:
            ...
            return FaceComparisonResult(
                ...
                model_used=RecognitionModel(model_used) if model_used else self.current_model_type,
    ...
    async def recognize_face(self, face_image: np.ndarray, 
                           gallery: FaceGallery,
                           model_name: Optional[str] = None,
                           top_k: int = 5) -> FaceRecognitionResult:
    ...
    async def add_face_to_database(self, person_id: str, face_image: np.ndarray, 
        ...
        try:
            ...
            self.logger.info(f"✅ Added face for {person_id} to database (total: {total_embeddings})")
            return True
    ...
    def create_gallery_from_database(self) -> FaceGallery:
        ...
        for person_id, embeddings in self.face_database.items():
            gallery[person_id] = {
                'name': person_id,
                'embeddings': [emb.vector for emb in embeddings if emb.vector is not None]
            }
