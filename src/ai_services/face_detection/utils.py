...
class BoundingBox:
    ...
    def from_array(cls, arr: Union[np.ndarray, 'BoundingBox']) -> 'BoundingBox':
        ...
        if isinstance(arr, np.ndarray):
            if len(arr) == 5:
                return cls(x1=float(arr[0]), y1=float(arr[1]), x2=float(arr[2]), 
                          y2=float(arr[3]), confidence=float(arr[4]))
            elif len(arr) == 6:
                return cls(x1=float(arr[0]), y1=float(arr[1]), x2=float(arr[2]), 
                          y2=float(arr[3]), confidence=float(arr[4]), class_id=int(arr[5]))
            else:
...
def calculate_face_quality(detection: BoundingBox, image_shape: Tuple[int, int], 
...
def validate_bounding_box(bbox: BoundingBox, image_shape: Tuple[int, int], 
                         relaxed_validation: bool = True) -> bool:
    """
...
def filter_detection_results(faces: List[FaceDetection], 
                           image_shape: Tuple[int, int], 
                           min_quality: float = 30.0,
                           relaxed_filtering: bool = True) -> List[FaceDetection]:
    """
...
def draw_detection_results(image: np.ndarray, 
                         ...
                         show_model: bool = False) -> np.ndarray:
    """
    ...
    for i, face in enumerate(detections):
        ...
        if show_confidence:
            ...
            cv2.putText(img_draw, conf_text, (x1, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
            y_offset -= 20
        
        if show_quality and face.quality_score is not None:
            quality_text = f"Q: {face.quality_score:.0f}"
            cv2.putText(img_draw, quality_text, (x1, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
            y_offset -= 20
        
        if show_model and face.model_used:
            model_text = f"Model: {face.model_used}"
            cv2.putText(img_draw, model_text, (x1, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1)
        
        # Face ID
        face_id = f"Face {i+1}"
        cv2.putText(img_draw, face_id, (x1, y2 + 15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
