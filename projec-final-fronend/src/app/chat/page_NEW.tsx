...
const ChatPage: React.FC = () => {
  ...
  useEffect(() => {
    if (lastWebSocketMessage && selectedConversation) {
      const data = JSON.parse(lastWebSocketMessage);
      
      // If it's a message for the current conversation
      if (data.type === 'new_message' && 
          (data.sender_id === selectedConversation.user.id || 
           data.receiver_id === selectedConversation.user.id)) {
        setMessages(prev => {
  ...
  }, [lastWebSocketMessage, selectedConversation]);
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          <div style={{
            ...
            backgroundColor: isOwn ? colors.light.primary : '#f5f5f5',
            color: isOwn ? '#ffffff' : colors.light.textPrimary,
            maxWidth: '100%',
            wordWrap: 'break-word',
            boxShadow: `0 1px 2px ${colors.light.shadowLight}`,
            position: 'relative'
          }}>
            <Text style={{ 
              color: isOwn ? '#ffffff' : colors.light.textPrimary,
            ...
            <>
              ...
              <div style={{ 
                ...
                <div style={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  />
                  <div>
                    <Text strong style={{ color: colors.light.textPrimary, fontSize: '16px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                      {selectedConversation.user.isOnline ? (
                        <>
                          ...
                          <Text style={{ ...textStyle.caption, color: colors.light.success }}>
                            ออนไลน์
                          </Text>
                        </>
                      ) : (
                        <Text style={{ ...textStyle.caption, color: colors.light.textTertiary }}>
                          ...
                          {selectedConversation.user.lastSeen ? 
                            dayjs(selectedConversation.user.lastSeen).fromNow() : 
                            'ออฟไลน์'
                          }
                        </Text>
                      )}
